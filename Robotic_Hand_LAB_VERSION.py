...
BLINK_WINDOW   = 3.0         # seconds — duration of each blink interval
BLINK_REST     = 1.5         # seconds — pause between windows

REPLAY_FILE    = "eeg_20260306_160721.csv"
...
def main():
    ...
    # ── Window state ──
    window_state = ['rest']   # 'rest', 'open', 'classifying'
    window_start = [0.0]
    ...
    def classifier_thread():
        while not stop_ev.is_set():
            ...
            # REST phase — short pause between windows
            window_state[0] = 'rest'
            ...
            # BLINK WINDOW — open for user to blink
            window_state[0] = 'open'
            window_start[0] = time.time()
            time.sleep(BLINK_WINDOW)

            if stop_ev.is_set():
                break

            # CLASSIFYING — grab buffer and predict
            window_state[0] = 'classifying'
            ...
            if eeg_snap is None:
                print("Buffer not full yet, skipping window.")
                continue
            ...
            try:
                ...
                result_q.put((pred, confidence, time.time()))
    ...
    while running:
        ...
        ws = window_state[0]
        if ws == 'open':
            elapsed  = time.time() - window_start[0]
            win_frac = min(1.0, elapsed / BLINK_WINDOW)
            win_col  = GREEN
            win_txt  = f"BLINK NOW  {BLINK_WINDOW - elapsed:.1f}s"
        elif ws == 'rest':
            win_frac = 0.0
            win_col  = GRAY
            win_txt  = "resting..."
        ...
        screen.blit(fs_small.render("BLINK WIN", True, GRAY), (sx+10, sy+80))
        pygame.draw.rect(screen, (20,30,50), (sx+85, sy+82, sw-100, 12), border_radius=4)
        pygame.draw.rect(screen, win_col,
                         (sx+85, sy+82, int((sw-100)*win_frac), 12), border_radius=4)
        screen.blit(fs_small.render(win_txt, True, win_col), (sx+85, sy+98))
        ...
        # Glow ring — pulses green when blink window is open
        rr = 125
        if ws == 'open':
            pulse    = 0.5 + 0.5 * np.sin(time.time() * 6)
            glow_col = lerp_col(GREEN, CYAN, pulse)
        else:
            glow_col = state_col
        rs = pygame.Surface((rr*2+20, rr*2+20), pygame.SRCALPHA)
        pygame.draw.circle(rs, glow_col+(15,), (rr+10, rr+10), rr)
        pygame.draw.circle(rs, glow_col+(40,), (rr+10, rr+10), rr, 2)
        ...
        # Blink prompt above hand
        if ws == 'open':
            prompt = fs_med.render(">> BLINK NOW <<", True, GREEN)
            screen.blit(prompt, (hcx - prompt.get_width()//2, hcy - rr - 40))
        elif ws == 'rest':
            prompt = fs_med.render("get ready...", True, GRAY)
            screen.blit(prompt, (hcx - prompt.get_width()//2, hcy - rr - 40))
        else:
            prompt = fs_med.render("reading signal...", True, YELLOW)
            screen.blit(prompt, (hcx - prompt.get_width()//2, hcy - rr - 40))
